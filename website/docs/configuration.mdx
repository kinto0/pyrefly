...
On the command-line, when no `pyrefly.toml` or `[tool.pyrefly]` section governs the file being checked, Pyrefly synthesizes a configuration on the fly:

- If a `mypy.ini`, `pyrightconfig.json`, or `[tool.mypy]` / `[tool.pyright]` section in `pyproject.toml` is found nearby, Pyrefly automatically migrates it and uses the resulting settings (the same migration `pyrefly init` would commit to disk).
- Otherwise Pyrefly uses the same [**basic preset**](#preset-basic) the IDE uses. This enables only high-confidence error kinds (syntax errors, missing imports, division by zero, etc.). Other diagnostics are silenced.

For the IDE equivalent of this behavior, including how to override it, see [`python.pyrefly.typeCheckingMode`](../IDE#pythonpyreflytypecheckingmode).
...
A minimal-checking preset used by default in projects without a Pyrefly configuration and in the LSP. Only the highest-confidence diagnostics (crashes and clearly broken code) are active. Every other error kind is silenced.
...
- Enables (as errors): [`division-by-zero`](./error-kinds.mdx#division-by-zero), [`invalid-syntax`](./error-kinds.mdx#invalid-syntax), [`missing-import`](./error-kinds.mdx#missing-import), [`parse-error`](./error-kinds.mdx#parse-error), [`unexpected-keyword`](./error-kinds.mdx#unexpected-keyword), [`unknown-name`](./error-kinds.mdx#unknown-name), [`invalid-annotation`](./error-kinds.mdx#invalid-annotation), [`not-async`](./error-kinds.mdx#not-async), [`unused-coroutine`](./error-kinds.mdx#unused-coroutine)
...
- Sets [`check-unannotated-defs`](#check-unannotated-defs) `= false`, [`infer-return-types`](#infer-return-types) `= "never"`
...
- Enables (as errors): [`implicit-any`](./error-kinds.mdx#implicit-any) (covers every implicit-`Any` sub-kind), [`missing-override-decorator`](./error-kinds.mdx#missing-override-decorator), [`unused-ignore`](./error-kinds.mdx#unused-ignore)

#### Preset: `all`
:::info
This is a new preset in *upcoming* Pyrefly version 1.1.0. It is not yet available in the current stable version.
:::
...
in training code. When `true`, all `pytorch-efficiency-lint-*` error kinds are
enabled at `warn` severity. You can override individual lint severities in
[`[errors]`](#errors).

- Type: `bool`
- Default: `false`
- Flag equivalent: `--pytorch-efficiency-lints`
...
Enable tensor shape type inference. Supports both native (`Tensor[N, M]`) and
jaxtyping (`Float[Tensor, "batch channels"]`) syntax.

- Type: `bool`
- Default: `false`
- Flag equivalent: `--tensor-shapes`
