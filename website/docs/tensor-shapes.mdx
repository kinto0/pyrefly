...
**`Int[X]`** bridges runtime integer values to the type level. When
`x: Tensor[3, 4]`, then `x.shape` has type `tuple[Int[3], Int[4]]` — you
can extract dimensions from tensors and use them to construct new ones.
Arithmetic works too: if `a: Int[3]` and `b: Int[4]`, then
`a * b: Int[12]`.
...
`Int` encodes symbolic shapes at the type level throughout PyTorch, covering modules as well as tensors.

**Arithmetic on type variables** lets you write custom shape transforms:

```python
def custom_rand_tensor[A, B](a: Int[A], b: Int[B]) -> Tensor[(A + B) // 2]:
