...
**`SymInt[X]`** bridges runtime integer values to the type level. When
`x: Tensor[3, 4]`, then `x.shape` has type `tuple[SymInt[3], SymInt[4]]` — you
can extract dimensions from tensors and use them to construct new ones.
Arithmetic works too: if `a: SymInt[3]` and `b: SymInt[4]`, then
`a * b: SymInt[12]`.
...
`SymInt` encodes symbolic shapes at the type level throughout PyTorch, covering modules as well as tensors.

**Arithmetic on type variables** lets you write custom shape transforms:

```python
def custom_rand_tensor[A, B](a: SymInt[A], b: SymInt[B]) -> Tensor[(A + B) // 2]:
