...
last_dim = hidden_units[-1]  # type is int, not SymInt[128]
```

Fix: add an explicit `SymInt` field to the config:
...
class DynamicMLP[InDim, OutDim](nn.Module):
    def __init__(self, in_dim: SymInt[InDim], out_dim: SymInt[OutDim],
...
- **Parameterized configs** propagate dimensions across modules without
  threading every `SymInt` through every constructor.
...
- **Dynamic construction** (`Sequential(*list)`, factory functions,
  `list[int]` access) breaks tracking — restructure or add explicit `SymInt`
  fields.
