...
- [`implicit-any-variable`](#implicit-any-variable)
...
A lambda whose parameter or return type cannot be inferred is treated as an implicit `Any`, hiding type errors in its body and at its call sites.
...
## implicit-any-variable

Default severity: `ignore`

A variable assigned a value of unknown type without an explicit annotation is inferred as an implicit `Any`, which hides type errors wherever it is used. Adding an annotation gives the variable a concrete type.
...
y = untyped(1)  # the type of `y` is unknown [implicit-any-variable]
y2: int = untyped(1)  # OK: `y2` is explicitly annotated
```
