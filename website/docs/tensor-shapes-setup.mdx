...
The fixtures also provide the `shape_extensions` package, which exports `SymInt` — the
...
You can also guard `Tensor` and `SymInt` under `TYPE_CHECKING` if you prefer
...
### Option 2: `shape_extensions.SymIntVar` (runtime-compatible)
...
syntax at runtime without crashing. It also provides a `SymIntVar` that
supports arithmetic (`N + 1` returns `self` instead of raising `TypeError`).

Use old-style generics with `shape_extensions.SymIntVar`:
...
class Linear(nn.Module):
    def __init__(self, n: SymInt[N], m: SymInt[M]):
        ...
```
...
description: Experiment with SymInt arithmetic, TYPE_CHECKING imports, and building typed tensors — no setup required.
```
...
class TwoLayerNet[InDim, HidDim, OutDim](nn.Module):
    def __init__(
        self,
        in_dim: SymInt[InDim],
        hid_dim: SymInt[HidDim],
        out_dim: SymInt[OutDim],
    ):
        super().__init__()
...
<img src="/img/tensor-shapes/nanogpt-forward-signature.png" alt="NanoGPT forward signature with x.size() unpacking to b: SymInt[B], t: SymInt[T], c: SymInt[NEmbedding]" />
