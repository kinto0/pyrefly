...
### Option 2: `shape_extensions.SymVar` (runtime-compatible)
...
syntax at runtime without crashing. It also provides a `SymVar` that
supports arithmetic (`N + 1` returns `self` instead of raising `TypeError`).

Use old-style generics with `shape_extensions.SymVar`:
