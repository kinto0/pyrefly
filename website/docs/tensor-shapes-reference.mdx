---
title: API Reference
description: Complete reference for SymInt, Tensor, and tensor shape type system APIs.
---
...
description: Experiment with SymInt operators, type-level arithmetic in shapes, and concatenation.
```

## `SymInt[X]`

`SymInt[X]` is a type constructor that bridges runtime integer values to
type-level symbols. It is defined in the `shape_extensions` package.

### Basics

`SymInt[X]` denotes the type of an integer value whose type-level identity is
`X`. For example:

- `SymInt[5]` is the type of the literal `5`
- `SymInt[N]` (where `N` is a type variable) is the type of an integer whose
  value is bound to `N` at the type level

`SymInt` is a subtype of `int`, so `SymInt` values can be used anywhere `int` is
expected. However, the reverse is not true — passing a plain `int` where
`SymInt[X]` is expected loses tracking.

### Arithmetic

Arithmetic on `SymInt` values produces `SymInt` results with the corresponding
type-level expression:

| Expression | Type |
|-----------|------|
| `a + b` where `a: SymInt[A]`, `b: SymInt[B]` | `SymInt[A + B]` |
| `a - b` | `SymInt[A - B]` |
| `a * b` | `SymInt[A * B]` |
| `a // b` | `SymInt[A // B]` |
| `a ** b` | `SymInt[A ** B]` |

**Caution:** `int * SymInt` produces `Unknown` because the `int` side has no
type-level identity. Use `SymInt * SymInt` or literal `* SymInt` instead.

### `SymInt[X] | None`

For optional dimensions — parameters that may or may not be present — use
`SymInt[X] | None`. In the forward method, narrow with
`if param is not None:` to recover `SymInt[X]` inside the branch:

```python
class Attention[D, RK](nn.Module):
    def __init__(self, dim: SymInt[D], rank_k: SymInt[RK] | None = None):
        ...

    def forward[B, T](self, x: Tensor[B, T, D]) -> Tensor[B, T, D]:
        if self.rank_k is not None:
            # rank_k is SymInt[RK] here
            ...
```
...
| Pattern | Purpose |
|---------|---------|
| `def __init__(self, dim: SymInt[D])` | Accept a dimension as a constructor parameter |
...
| `self.head_dim = dim // n_head` | Compute a derived dimension (`SymInt[D // NHead]`) |
...
class-level SymInt `D`, use `Tensor[*Bs, D]` not `Tensor[*S]`.

### `.shape` and `.size()`

When `x: Tensor[B, C, H, W]`:

- `x.shape` has type `tuple[SymInt[B], SymInt[C], SymInt[H], SymInt[W]]`
- `x.size(0)` has type `SymInt[B]`
- `x.size()` has type `tuple[SymInt[B], SymInt[C], SymInt[H], SymInt[W]]`
