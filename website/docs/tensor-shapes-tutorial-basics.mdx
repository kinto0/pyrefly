...
By the end, you'll understand `SymInt`, `Tensor[...]`, class-level type
...
Both flow to sub-module constructors, so both must be `SymInt`, not `int`.
(`SymInt[X]` is a type that bridges a runtime integer value to a type-level
...
Make the dimension parameters into `SymInt[...]` and add class-level type
parameters:
...
see [Getting Started](./tensor-shapes-setup.mdx#option-2-shape_extensionssymintvar-runtime-compatible)
for the `shape_extensions.SymIntVar` import style, which requires old-style
generics.
:::

## Key concepts

- **`SymInt[X]`** bridges runtime integer values to type-level symbols. Constructor
  parameters that determine tensor dimensions should be `SymInt[X]`, not `int`.
