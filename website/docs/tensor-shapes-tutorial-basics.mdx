...
By the end, you'll understand `Int`, `Tensor[...]`, class-level type
...
Both flow to sub-module constructors, so both must be `Int`, not `int`.
(`Int[X]` is a type that bridges a runtime integer value to a type-level
...
Make the dimension parameters into `Int[...]` and add class-level type
parameters:
...
see [Getting Started](./tensor-shapes-setup.mdx#option-2-shape_extensionsintvar-runtime-compatible)
for the `shape_extensions.IntVar` import style, which requires old-style
generics.
:::

## Key concepts

- **`Int[X]`** bridges runtime integer values to type-level symbols. Constructor
  parameters that determine tensor dimensions should be `Int[X]`, not `int`.
