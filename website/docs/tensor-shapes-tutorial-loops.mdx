...
Notice that `n_layers` is `int`, not `SymInt` — it's an iteration count that
doesn't flow into any tensor dimension. Only values that determine tensor
shapes need to be `SymInt`.
...
- **`int` for iteration counts, `SymInt` for tensor dimensions.** `n_layers`
