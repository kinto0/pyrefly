...
def test_view_symbolic[N: SymIntVar, M: SymIntVar](
    ...
    The .size() method returns SymInt[N] and SymInt[M], which can be used
    in arithmetic operations and passed to .view() for shape transformation.
    """
    # Get both dimensions from input - these return SymInt[N] and SymInt[M]
    n = x.size(0)
    m = x.size(1)
    assert_type(n // 2, SymInt[N // 2])
    assert_type(m, SymInt[M])
