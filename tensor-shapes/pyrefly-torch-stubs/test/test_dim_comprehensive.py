...
def test_dim_typevar_parsing[N: SymVar](n: Dim[N]) -> None:
    """Dim[N] with type variable"""
    assert_type(n, Dim[N])


def test_dim_expression_parsing[N: SymVar](n: Dim[N]) -> None:
...
def test_dim_add_literal[N: SymVar](n: Dim[N]) -> None:
    """Addition: Dim + literal"""
    result = n + 2
    assert_type(result, Dim[N + 2])


def test_dim_add_dim[A: SymVar, B: SymVar](a: Dim[A], b: Dim[B]) -> None:
    """Addition: Dim + Dim"""
    result = a + b
    assert_type(result, Dim[A + B])


def test_dim_sub_literal[N: SymVar](n: Dim[N]) -> None:
    """Subtraction: Dim - literal"""
    result = n - 1
    assert_type(result, Dim[N - 1])


def test_dim_sub_dim[A: SymVar, B: SymVar](a: Dim[A], b: Dim[B]) -> None:
    """Subtraction: Dim - Dim"""
    result = a - b
    assert_type(result, Dim[A - B])


def test_dim_mul_literal[N: SymVar](n: Dim[N]) -> None:
    """Multiplication: Dim * literal"""
    result = n * 2
    assert_type(result, Dim[N * 2])


def test_dim_mul_dim[A: SymVar, B: SymVar](a: Dim[A], b: Dim[B]) -> None:
    """Multiplication: Dim * Dim"""
    result = a * b
    assert_type(result, Dim[A * B])


def test_dim_floordiv_literal[N: SymVar](n: Dim[N]) -> None:
    """Floor division: Dim // literal"""
    result = n // 2
    assert_type(result, Dim[N // 2])


def test_dim_floordiv_dim[A: SymVar, B: SymVar](a: Dim[A], b: Dim[B]) -> None:
    """Floor division: Dim // Dim"""
    result = a // b
    assert_type(result, Dim[A // B])


def test_dim_complex_expression[N: SymVar](n: Dim[N]) -> None:
...
def test_dim_nested_expression[A: SymVar, B: SymVar](a: Dim[A], b: Dim[B]) -> None:
    """Nested arithmetic"""
    result = (a + b) * 2
...
def dim_to_dim_same[N: SymVar](x: Dim[N]) -> Dim[N]:
    """Dim[N] <: Dim[N]"""
    return x
...
def dim_to_int[N: SymVar](x: Dim[N]) -> int:
...
def dim_expression_subtype[N: SymVar](x: Dim[N + N]) -> Dim[N * 2]:
    """Dim[N + N] <: Dim[N * 2] - after simplification these are equal"""
    return x


def dim_double_half[N: SymVar](x: Dim[(N + N) // 2]) -> Dim[N]:
...
def identity_dim[X: SymVar](x: Dim[X]) -> Dim[X]:
...
def test_dim_binding_typevar[N: SymVar](n: Dim[N]) -> None:
    """Binding type var to another type var"""
    result = identity_dim(n)
    assert_type(result, Dim[N])


def test_dim_binding_expression[A: SymVar, B: SymVar](a: Dim[A], b: Dim[B]) -> None:
...
def double_dim[X: SymVar](x: Dim[X]) -> Dim[X * 2]:
...
def test_double_dim_typevar[N: SymVar](n: Dim[N]) -> None:
    """Double a symbolic dimension"""
    result = double_dim(n)
    assert_type(result, Dim[N * 2])


def half_dim[X: SymVar](x: Dim[X]) -> Dim[X // 2]:
...
def test_half_dim_typevar[N: SymVar](n: Dim[N]) -> None:
...
def add_dims[A: SymVar, B: SymVar](a: Dim[A], b: Dim[B]) -> Dim[A + B]:
    """Add two dimensions"""
    return a + b
...
def test_add_dims_typevars[X: SymVar, Y: SymVar](x: Dim[X], y: Dim[Y]) -> None:
    """Add symbolic dimensions"""
    result = add_dims(x, y)
    assert_type(result, Dim[X + Y])


def test_add_dims_mixed[N: SymVar](n: Dim[N]) -> None:
...
def two_dims_same_var[X: SymVar](first: Dim[X], second: Dim[X]) -> Dim[X]:
...
def test_two_dims_same_typevar[N: SymVar](n: Dim[N]) -> None:
    """Same typevar for both"""
    result = two_dims_same_var(n, n)
    assert_type(result, Dim[N])


def with_derived[X: SymVar](first: Dim[X], second: Dim[X // 2]) -> Dim[X]:
    """Second arg uses derived dimension"""
    return first


def test_derived_binding[N: SymVar](n: Dim[N]) -> None:
...
def test_derived_with_simplification[A: SymVar](a: Dim[A]) -> None:
