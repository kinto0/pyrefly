...
class FeedForward(nn.Module, Generic[D, IntermediateSize]):
    def __init__(
    ...
    def forward(self, x: Tensor[[B, T, D]]) -> Tensor[[B, T, D]]:
...
class RMSNorm(nn.Module, Generic[D]):
    ...
    def forward(self, x: Tensor[[*Elements[Bs], D]]) -> Tensor[[*Elements[Bs], D]]:
        output = self._norm(x.float()).type_as(x)
        return output * self.weight


class Attention(nn.Module, Generic[D, NHead, NLocalHeads]):
...
class TransformerBlock(nn.Module, Generic[D, NHead, IntermediateSize, NLocalHeads]):
