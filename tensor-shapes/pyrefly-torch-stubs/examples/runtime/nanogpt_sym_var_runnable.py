...
Runnable variant using TypeVar from shape_extensions and Generic instead
...
class CausalSelfAttention(nn.Module, Generic[NEmbedding, NHead, BlockSize]):
    ...
    def forward(self, x: Tensor[[B, T, NEmbedding]]) -> Tensor[[B, T, NEmbedding]]:
        b, t, c = (
            x.size()
...
class MLP(nn.Module, Generic[NEmbedding]):
...
class Block(nn.Module, Generic[NEmbedding, NHead, BlockSize]):
...
class GPTConfigArgs(
    TypedDict, Generic[VocabSize, BlockSize, NEmbedding, NHead, NLayer]
):
...
class TransformerModules(TypedDict, Generic[VocabSize, BlockSize, NEmbedding, NHead]):
...
class GPT(nn.Module, Generic[VocabSize, BlockSize, NEmbedding, NHead, NLayer]):
